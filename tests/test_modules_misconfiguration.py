...
def _context(headers, final_url="https://example.com/", tls=None):
    discovery = DiscoveryResult(
        target_url=final_url, final_url=final_url, ok=True, status_code=200,
        headers=headers, tls=tls or TlsInfo(),
    )
    return ScanContext(target=ScanTarget(url=final_url), http=None, settings=None, discovery=discovery)


async def test_flags_all_missing_security_headers():
    module = MisconfigurationModule()
    findings = await module.run(_context(headers={}))
...
async def test_present_headers_are_not_flagged():
    ...
    findings = await module.run(_context(headers=headers))
    assert findings == []


async def test_hsts_not_required_over_plain_http():
    module = MisconfigurationModule()
    findings = await module.run(_context(headers={}, final_url="http://example.com/"))
...
async def test_unverifiable_tls_certificate_is_flagged():
    ...
    findings = await module.run(_context(headers={}, tls=tls))
